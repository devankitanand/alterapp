...
const Createpost = () => {
    ...
    const handleFileChange = (e, type) => {
        if (type === 'photo' && e.target.files.length > 4) {
            toast.error("You can only select up to 4 photos.");
            return;
        }
            ...
            {Isnext ? (
                <>
                    ...
                    <div className='galleryactions ml-5 mt-5'>
                        <div>
                            <div onClick={() => triggerFileInput('photo')} className="clickable font-bold">Photos ({photoCount})</div>
                            <div onClick={() => triggerFileInput('video')} className="clickable font-bold">Videos ({videoCount})</div>
                            <div className="clickable font-bold">Camera</div>
                        </div>
                        <input
                            type="file"
                            accept="image/*"
                ...
                <>
                    ...
                    <div className='flex justify-center'>
                        {isUploading ? (
                            <ClipLoader size={24} color={"#000"} loading={isUploading} />
                        ) : (
